import styled from 'styled-components';
import {
  MatchInvitationsWrapper,
  MatchInvitationsCardWrapper,
  MatchInvitationsCardImage,
  MatchInvitationsInfo,
  MatchInvitationsName,
  MatchInvitationsDescription,
} from '../MatchInvitations/matchInvitations.styles';

import { ScheduleSideOption, CancelButton } from '../Schedule/schedule.styles';

export const MatchInvitesSentWrapper = styled(MatchInvitationsWrapper)``;
export const MatchInvitesSentCardWrapper = styled(
  MatchInvitationsCardWrapper
)``;
export const MatchInvitesSentCardImage = styled(MatchInvitationsCardImage)``;
export const MatchInvitesSentInfo = styled(MatchInvitationsInfo)``;
export const MatchInvitesSentName = styled(MatchInvitationsName)``;
export const MatchInvitesSentDescription = styled(
  MatchInvitationsDescription
)``;
export const MatchInvitesSideOption = styled(ScheduleSideOption)``;
export const MatchInvitesCancelButton = styled(CancelButton)``;
